...
/////////////////////////////////////////////// PIT EDIT///////////////////////////////////////////////
  ...
  if (!pitEntry->hasInRecords()) {
    if (m_csFromNdnSim == nullptr) {
      ...
      if (match != nullptr) {
        this->onContentStoreHit(inFace, pitEntry, interest, *match); /////////////////  pour virifie l'existance du Y
                              ...
                              const Interest& interest)
{
 ...
 /***********************************"CacheMisses" Cstracer*************************************************/
   misses=misses+1; 
     Name Importantname1="/Important/";
	 Name nameInt1= interest.getName(); 
     if(Importantname1.isPrefixOf(nameInt1)){
   std::string const outputfilemissImp("/home/mohammed/Desktop/ndnSIM/ns-3/CsTracermissImp.csv");
   std::ofstream monFlux1(outputfilemissImp.c_str(), std::ios::app);
  	   
   monFlux1  <<ns3::Simulator::Now().GetSeconds ()<<"\t"<<"CacheMisses"<<"\t"<<1<<"\n"; }
   else
   {std::string const outputfilemissNotImp("/home/mohammed/Desktop/ndnSIM/ns-3/CsTracermissNotImp.csv");
   std::ofstream monFlux2(outputfilemissNotImp.c_str(), std::ios::app);
  	   
   monFlux2  <<ns3::Simulator::Now().GetSeconds ()<<"\t"<<"CacheMisses"<<"\t"<<1<<"\n";}
                             ...
                             const Interest& interest, const Data& data)
{
 ...
 /***********************************"CacheHits" Cstracer*************************************************/
     Name Importantname2="/Important/";
     Name nameInt2= interest.getName();
    if(Importantname2.isPrefixOf(nameInt2)){
   std::string const outputfileCHImp("/home/mohammed/Desktop/ndnSIM/ns-3/csTracerhitImortant.csv");
   std::ofstream monFlux3(outputfileCHImp.c_str(), std::ios::app);
   
   /* hit++;   
   if(ns3::Simulator::Now().GetSeconds ()>=(m_periodhit+ 0.5)){
	monFlux << inFace.getId() <<"\t"<<ns3::Simulator::Now().GetSeconds () <<"\t"<<"CacheHits"<<"\t"<<hit<<"\n";     
	   hit=0;
	   m_periodhit=ns3::Simulator::Now().GetSeconds();}*/
	
	 
	monFlux3  <<ns3::Simulator::Now().GetSeconds ()<<"\t"<<"CacheHits"<<"\t"<<1<<"\n";    }
	else{
   std::string const outputfileCHNotImp("/home/mohammed/Desktop/ndnSIM/ns-3/csTracerhitNotImortant.csv");
   std::ofstream monFlux4(outputfileCHNotImp.c_str(), std::ios::app);
   
	monFlux4  <<ns3::Simulator::Now().GetSeconds ()<<"\t"<<"CacheHits"<<"\t"<<1<<"\n";    }
  /******************************************************************************************************/
